...
def build_comparable_report(company_type: CompanyType, values: Mapping[str, MetricValue], metrics: MetricPack, market_data: Mapping[str, object]) -> ComparableReport:
    ...
    rows = [
        compare_multiple(name, company_multiples.get(name), peer_medians.get(name), peer_source(name, market_data))
    ...
    confidence = min(1.0, len(usable) / max(COMPARABLES.minimum_peer_metrics, len(selected)))
    overall = sum(row.score for row in usable) / len(usable) if usable else 0.0
    return ComparableReport(rows, overall, confidence, comparable_summary(overall, confidence, len(usable)))
...
def compare_multiple(name: str, company_value: float | None, peer_median: float | None, source: str) -> ComparableMetric:
    ...
    return ComparableMetric(name, company_value, peer_median, premium, score, source, interpretation_for(premium))
...
def selected_multiples(company_type: CompanyType) -> tuple[str, ...]:
    ...
    return ("price_to_earnings", "ev_to_ebit", "ev_to_sales", "price_to_book")
...
def comparable_summary(score: float, confidence: float, usable_count: int) -> str:
...
def interpretation_for(premium: float | None) -> str:
    if premium is None:
